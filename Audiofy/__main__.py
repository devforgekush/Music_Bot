...
async def init():
    if (
        ...
        LOGGER(__name__).error("ðŸš« String Session Missing! Please configure at least one Pyrogram session string.")
        exit()
    await sudo()
    try:
    ...
    LOGGER("Audify.plugins").info("✅ All modules successfully loaded. @devforgekush is ready to serve 🎶")
    await userbot.start()
    await Audify.start()
    try:
    ...
    LOGGER("Audify").info(
    "🎧 @devforgekush Music Bot started successfully.\n🛡️ Developed and maintained by @devforgekush 💻\nChannel: @devforgekush"
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("Audify").info("🛑 @devforgekush Music Bot has stopped. See you soon! 👋")
